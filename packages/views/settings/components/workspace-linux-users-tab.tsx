...
export function WorkspaceLinuxUsersTab() {
  ...
  const target = mode === "existing" && selected && eligibility(selected) === "available"
    ...
    : mode === "new" && machines.some((m) => m.id === computerId && m.enabled)
      ...
      : null;
  const hasCredentials = !!data.settings.data?.settings.multica_pat;
  ...
  return (
    <SettingsTab title={t(($) => $.workspace_linux_users.title)}>
      <SettingsSection title={t(($) => $.workspace_linux_users.connected)}>
        {data.bindings.isPending ? <p role="status">{t(($) => $.computers.loading)}</p> : null}
        {connected.length === 0 &&
          !data.bindings.isPending ? <p className="text-muted-foreground">{t(($) => $.workspace_linux_users.none_connected)}</p> : null}
        <ul className="space-y-2">
          {connected.map((b) => (
            <li key={b.id} className="flex flex-wrap items-center justify-between gap-2 border-b py-2">
              <span>{machineName(b.computer_id)} · {b.username}
      ...
      <SettingsSection title={t(($) => $.workspace_linux_users.connect)}>
        {(data.machines.error || data.bindings.error || data.settings.error) &&
          <p role="alert" className="text-destructive">{data.machines.error?.message || data.bindings.error?.message || data.settings.error?.message}</p>}
        {!hasCredentials && !data.settings.isPending &&
          <p>{t(($) => $.workspace_linux_users.credentials_required)} <AppLink href={personalHref} className="underline">{t(($) => $.workspace_linux_users.open_personal)}</AppLink></p>}
        ...
        <form onSubmit={(event) => void submit(event)} className="max-w-xl space-y-4">
          {mode === "existing" ? (
            existing.length ? <fieldset className="space-y-2">
              <legend className="font-medium">{t(($) => $.workspace_linux_users.choose_existing)}</legend>
              {existing.map((b) => {
                const status = eligibility(b);
                return <div key={b.id} className="border-b py-2">
                  <label className="flex gap-3">
                    <input type="radio" name="linux-user" value={b.id} checked={selectedId === b.id} disabled={status !== "available"} onChange={() => setSelectedId(b.id)} />
                    <span className="min-w-0"><span className="block font-medium">{machineName(b.computer_id)} · {b.username}</span>
            ...
            <>
              <label className="block space-y-1"><span>{t(($) => $.computers.choose)}</span>
                <Select items={machines.filter((m) => m.enabled).map((m) => ({ value: m.id, label: m.name }))} value={computerId} onValueChange={(value) => setComputerId(value ?? "")}>
                  <SelectTrigger className="w-full" aria-label={t(($) => $.computers.choose)}><SelectValue placeholder={t(($) => $.computers.choose)} /></SelectTrigger>
                  <SelectContent>{machines.filter((m) => m.enabled).map((m) => <SelectItem key={m.id} value={m.id}>{m.name}</SelectItem>)}</SelectContent>
                </Select>
              </label>
              <label className="block space-y-1"><span>{t(($) => $.computers.username)}</span>
                <Input value={username} required pattern="[a-z_][a-z0-9_-]{0,31}" onChange={(event) => setUsername(event.target.value)} />
              </label>
            </>
          )}
          <label className="block space-y-1"><span>{t(($) => $.computers.password)}</span>
        ...
        <p className="text-sm text-muted-foreground"><AppLink href={personalHref} className="underline">{t(($) => $.workspace_linux_users.manage_personal)}</AppLink></p>
      </SettingsSection>
    </SettingsTab>
  );
}
