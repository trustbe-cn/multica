...
function PersonalComputersTab() {
  ...
  const [action, setAction] =
    useState<ComputerOperation["action"]>("create_account");
  const [error, setError] = useState("");
  const [notice, setNotice] = useState("");
  const availableMachines =
    data.machines.data?.filter((m) => m.enabled || action === "remove") ?? [];
  ...
  return (
    <SettingsTab title={t(($) => $.computers.personal_title)}>
      {(error || data.machines.error || data.bindings.error) && (
      ...
      <p>
        <AppLink
          href={`${paths.workspace(workspace?.slug ?? "").settings()}?tab=credentials`}
          className="underline"
        >
      ...
      >
        <form onSubmit={submit} className="space-y-4">
          ...
          <p>
            ...
            {workspaceTarget || workspace?.name || workspace?.id || "—"}
          </p>
          <label className="block space-y-1">
          ...
          <label className="block space-y-1">
            ...
            <Select
              items={(["create_account", "upgrade", "remove"] as const).map(
                (a) => ({ value: a, label: t(($) => $.computers.actions[a]) }),
              )}
              value={action}
              onValueChange={(v) => {
                if (v === "create_account" || v === "upgrade" || v === "remove")
                  setAction(v);
              }}
            >
              <SelectTrigger className="w-full">
              ...
              <SelectContent>
                {(["create_account", "upgrade", "remove"] as const).map((a) => (
          ...
          <Button
            ...
            disabled={
              busy ||
              !machine ||
              !availableMachines.some((m) => m.id === machine) ||
              !workspace
            }
      ...
      <SettingsSection
        title={t(($) => $.workspace_linux_users.choose_existing)}
      >
        <ul className="space-y-3" aria-live="polite">
          {data.bindings.data?.map((b) => (
            <BindingRow
              ...
              binding={b}
              machineName={
                data.machines.data?.find((m) => m.id === b.computer_id)?.name ??
                b.computer_id
              }
              userId={user?.id ?? ""}
              onConfigure={(nextAction) => {
                setMachine(b.computer_id);
                setUsername(b.username);
                setAction(nextAction);
                setWorkspaceTarget(
                  nextAction === "create_account" ? "" : b.workspace_id,
                );
                setPassword("");
                setNotice(t(($) => $.linux_user.prompt_password));
  ...
  return (
    <li className="rounded-lg border p-3 break-words">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <p>
          {machineName} · {binding.username} ·{" "}
          {t(
            ($) =>
              $.linux_user.states[
                binding.state as keyof typeof $.linux_user.states
              ] ?? $.linux_user.unknown,
          )}
        </p>
        <Button
      ...
      <p className="break-all text-sm text-muted-foreground">
        {t(($) => $.admin.linux_user_workspace)}: {binding.workspace_id}
      </p>
      {binding.last_error && (
        <p className="text-destructive">{binding.last_error}</p>
      )}
      {open && (
        <LinuxUserDetail
          userId={userId}
        ...
        >
          {(busy) =>
            ["ready", "failed", "pending"].includes(binding.state) ? (
              <BindingRuntimes
                bindingId={binding.id}
                userId={userId}
                busy={busy}
        ...
        <div className="flex flex-wrap items-center gap-2">
          {supportsVersion && (
            <Input
              ...
              aria-label={t(($) => $.admin.runtimes_version_label, {
