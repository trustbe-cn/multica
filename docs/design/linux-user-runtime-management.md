...
当前接口（详细操作契约见[远端操作与清理](computer-remote-operations.md)）：

| 接口 | 用途 | 授权 |
| --- | --- | --- |
| `GET /api/me/computer-bindings/{id}/runtimes` | 读取绑定对应 Linux User 的 CLI 资产快照 | 当前用户拥有该绑定 |
| `POST /api/me/computer-bindings/{id}/discover` | 后台检查 CLI 并通知 daemon 刷新 | 当前用户拥有可操作的绑定 |
...
详情视图、持久操作历史、错误恢复、资产信息和 remove/archive/delete_user 的区别见[Linux 用户远端操作、资产与清理](computer-remote-operations.md)。
