...
账号创建顺序为：安装系统包 → `useradd` 使用已安装的 zsh → `chpasswd`。`create_account` 到此完成，标记账号已验证、绑定待配置；写凭据和启动 daemon 是独立动作。原 `provision` API 仍继续写个人配置并启动 daemon。

Oh My Zsh 属于可选 shell 增强，不随建号自动安装，也不会因其 GitHub 下载失败而回滚账号。需要时由用户建号后自行安装。

- 当前支持 Debian/Ubuntu 的 `apt-get` 路径，系统包包括 zsh、htop、curl、git。连接检查验证工具存在，不保证包仓库、网络或软件源可用。
- apt 失败发生在 `useradd` 之前，无账号可删除。密码设置失败时，创建脚本尝试删除刚建的账号。
- `CreateUser` 成功后，写配置或安装 daemon 失败由外层流程补偿删除新账号。复用的既有账号不走该删除补偿。
- apt 有 120 秒进程组执行预算，超时先发送 SIGTERM，再清理残留进程；建号 SSH 预算至少 8 分钟。
