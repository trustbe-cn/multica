...
5. **初始化契约已调整。**当前建号不再安装 Oh My Zsh；保留系统包、zsh、账号与密码设置，用户可以建号后自行配置 shell 增强。这样 GitHub 安装脚本下载失败不会回滚账号。上文的 oh-my-zsh 超时与回滚说明是前两轮实现记录，当前行为以[建号与个人凭据指南](linux-user-credentials-guide.md)为准。
