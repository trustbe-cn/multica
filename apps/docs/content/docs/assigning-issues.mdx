...
During the run, the agent manages the issue's status by judging, at the end of each turn, what its work changed: delivery moves the issue to `in_review`, work continuing beyond the turn keeps it `in_progress`, and research or discussion leaves the status untouched — these actions appear under the agent's identity in the timeline. Task lifecycle and issue status are separate: a task completing does not by itself change the issue status.
