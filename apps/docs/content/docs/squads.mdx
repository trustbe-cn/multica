...
5. **The dispatch turn ends with the parent `in_progress`.** Same agent-managed status contract as a direct agent assignment — status is judged at the end of the turn, and dispatching members is not delivery, so the parent leaves `todo` for `in_progress` and stays there while the squad works.
