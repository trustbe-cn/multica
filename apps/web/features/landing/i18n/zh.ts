    ...
    entries: [
      {
        version: "0.4.44",
        date: "2026-09-14",
        title: "钉钉引用回复、Issue 状态设置改版、更安全的评论删除、更稳的飞书扫码绑定",
        changes: [],
        features: [
          "钉钉群里的回复会引用你发的那条消息。",
          "钉钉用表情回应标记处理中，完成后再标一次。",
          "钉钉的命令确认会保留你输入的内容，并能点进新建的 Issue。",
          "设置里的 Issue 状态按未开始、已开始、已完成、已关闭分组。",
          "拖动状态（含内置状态）即可决定它在看板、列表和状态菜单里的位置。",
          "自定义状态可以自己挑图标形状。",
          "DeepSeek Harness 桌面版无需手动配置即可作为运行时被发现。",
        ],
        improvements: [
          "归档状态时会告诉你还有多少 Issue 在用，并可直接跳过去查看。",
          "DeepSeek Harness 运行时起不来时，会说明缺了什么。",
          "命令行里能直接看到每个 Autopilot 上次运行的结果。",
        ],
        fixes: [
          "删除一条评论时，别人在下面留的回复会保留。",
          "已删除的回复不再留下痕迹，链接过去会落在旁边那条消息上。",
          "在 Windows 上打包的技能压缩包，导入后可以正常使用。",
          "Windows 和自定义 Hermes 目录里的技能会出现在技能页和导入弹窗。",
          "企业微信不再在文件前发一条空消息，文件没发出去也会明确说明。",
          "飞书扫码绑定的二维码有效期是完整的一小时，也不会在第一次查询时失效。",
          "版本较旧的桌面端，在服务端更新后仍能保留自定义状态。",
          "两条评论同时指派同一个智能体时会并入一次运行，而不是报错。",
          "Claude 运行结束时缺少最终用量，不会再把 token 数算多。",
        ],
      },
      {
        version: "0.4.43",
