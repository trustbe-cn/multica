      ...
      entries: [
        {
          version: "0.4.44",
          date: "2026-09-14",
          title: "DingTalk 인용 답장, 새로워진 Issue 상태 설정, 더 안전한 댓글 삭제, 더 확실한 Lark 연동 QR",
          changes: [],
          features: [
            "DingTalk 그룹에서의 답장에 내가 보낸 메시지가 인용됩니다.",
            "DingTalk이 처리 중임을 리액션으로 알리고, 끝나면 한 번 더 표시합니다.",
            "DingTalk 명령 확인에 입력한 내용이 남고, 만들어진 Issue로 바로 갈 수 있습니다.",
            "설정의 Issue 상태가 시작 전, 시작됨, 완료, 종료로 묶입니다.",
            "상태를 끌어다 놓으면 보드와 목록, 상태 메뉴에서의 자리가 정해집니다(기본 상태도 포함).",
            "사용자 지정 상태에 원하는 아이콘 모양을 고를 수 있습니다.",
            "DeepSeek Harness Desktop이 별도 설정 없이 런타임으로 잡힙니다.",
          ],
          improvements: [
            "상태를 보관할 때 아직 쓰고 있는 Issue가 몇 개인지 알려 주고, 그곳으로 이동할 수 있습니다.",
            "DeepSeek Harness 런타임이 시작되지 않으면 무엇이 빠졌는지 알려 줍니다.",
            "명령줄에서 각 오토파일럿의 지난 실행 결과를 볼 수 있습니다.",
          ],
          fixes: [
            "댓글을 지워도 그 아래 달린 답글은 그대로 남습니다.",
            "지워진 답글은 아무 흔적도 남기지 않고, 그 링크는 옆 메시지로 이동합니다.",
            "Windows에서 만든 스킬 압축 파일이 바로 쓸 수 있는 스킬로 들어옵니다.",
            "Windows와 직접 정한 Hermes 홈의 스킬이 스킬 탭과 가져오기 창에 나타납니다.",
            "WeCom이 파일 앞에 빈 메시지를 보내지 않고, 파일이 실패하면 그렇게 알려 줍니다.",
            "Lark 연동 QR 코드가 한 시간을 다 채우고, 첫 확인에서 끊기지 않습니다.",
            "버전이 낮은 데스크톱 앱도 서버 업데이트 뒤에 사용자 지정 상태를 유지합니다.",
            "같은 에이전트에게 동시에 들어온 댓글 두 개가 실패하지 않고 한 실행으로 합쳐집니다.",
            "최종 집계 없이 끝난 Claude 실행이 사용한 토큰을 부풀려 보여 주지 않습니다.",
          ],
        },
        {
          version: "0.4.43",
